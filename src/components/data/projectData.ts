const projectsData = [
  {
    title: "Web Development Uncharted: React and TypeScript Expedition",
    date: "Oct 2023",
    desc: "This project studies the potential improvements fuzzy logic could make to robotic controlled systems and assess its potential future uses for systems similar to the 3PI+ robot.",
    imageSrc: "/imgs/project/SimulinkModel.png",
    active: true,
  },
  {
    title: "Exploring Robotics: 3pi+ Line Following Adventure",
    date: "Mar 2023",
    desc: "This is where a breif desciprtion of what the project was about the goals and the outcome of the project will go. its aim is to draw the reader into it more.",
    imageSrc: "/imgs/project/3piRobot.jpeg",
    active: false,
  },
  {
    title: "SolarBot: Building an Autonomous Solar Robot",
    date: "May 2023",
    desc: "This is where a breif desciprtion of what the project was about the goals and the outcome of the project will go. its aim is to draw the reader into it more.",
    imageSrc: "/imgs/project/Sally.jpeg",
    active: false,
  },
  {
    title: "Investigation of PID control using conventional and fuzzy logic",
    date: "May 2023",
    desc: "This project studies the potential improvements fuzzy logic could make to robotic controlled systems and assess its potential future uses for systems similar to the 3PI+ robot.",
    imageSrc: "/imgs/project/kpSurface.png",
    active: false,
  },

  {
    title:
      "Designing and implementation of a control system for a Quanser helicopter",
    date: "May 2022",
    desc: "This project studies the potential improvements fuzzy logic could make to robotic controlled systems and assess its potential future uses for systems similar to the 3PI+ robot.",
    imageSrc: "/imgs/project/3dof.jpeg",
    active: false,
  },

  {
    title:
      "Design and Implementation of a Bi-Directional Charger for an 800V Electric Vehicle",
    date: "May 2022",
    desc: "This project studies the potential improvements fuzzy logic could make to robotic controlled systems and assess its potential future uses for systems similar to the 3PI+ robot.",
    imageSrc: "/imgs/project/SimulinkModel.png",
    active: false,
  },
];

export default projectsData;
